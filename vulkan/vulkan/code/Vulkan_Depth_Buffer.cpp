...
void vulkan::data::Depth_Buffer::Init(Utility::Int_Vec2 in_win,PhysicalDevice* in_phyDevice, Device* in_Device)
{
	...
	/* Create image */
	vulkan::Error::Window(vkCreateImage(in_Device->device, &image_info, NULL, &this->image),"ƒCƒ[ƒW‚Ìì¬‚ÉŽ¸”s");
	...
	/* Allocate memory */
	vulkan::Error::Window(vkAllocateMemory(in_Device->device, &mem_alloc, NULL, &mem),"[“xƒƒ‚ƒŠ‚ÌŠm•Û‚ÉŽ¸”s");

	/* Bind memory */
	vulkan::Error::Window(vkBindImageMemory(in_Device->device, this->image, mem, 0),"[“xƒoƒCƒ“ƒhƒƒ‚ƒŠ‚Ìì¬‚ÉŽ¸”s");

	/* Create image view */
	view_info.image = this->image;
	vulkan::Error::Window(vkCreateImageView(in_Device->device, &view_info, NULL, &this->view),"[“xƒCƒ[ƒWƒrƒ…[‚Ìì¬‚ÉŽ¸”s");
}
